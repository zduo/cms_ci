$(function() {
	...
	$('#tabs ul li a').click(function(){
		...
		//$('#tabs div').hide();
		//$(currentTab).show();
		//y=document.getElementById('div的id').style.top
		//console.log($(currentTab).position());
		//console.log($(currentTab).position().top);
				
		//$('#tab_select').animate({ top: $(currentTab).position().top }, 800);
		//$(window).scrollTop(3000);
		//window.location.hash=$(currentTab).name;
		$('body,html').animate({
			scrollTop: $(currentTab).position().top
		}, 800);
		
		//$('#tab_select').y =  $(currentTab).position().top;
		return false;
	});
	$('#h_tabs div').hide();
